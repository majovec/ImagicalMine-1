...
class Level implements ChunkManager, Metadatable{
	...
	/** Weather **/
	const WEATHER_IMAGICALMINE = 0;//Project Name , Have fun~
	const WEATHER_RAIN = 1;
	const WEATHER_THUNDER = 2;
	
	private $weather = self::WEATHER_IMAGICALMINE;
	private $rainprob = 0;
	private $raintime = [];
	private $rainfall = [];
	...
	private function generateWeather(){
		if($this->getServer()->getProperty("level-settings.weather.enable", true) && $this->getDimension() == self::DIMENSION_NORMAL){
			if($this->getWeather() === self::WEATHER_IMAGICALMINE){
				$random = mt_rand(1, 1000000);
				if($random <= $this->rainprob){
					Server::getInstance()->getPluginManager()->callEvent($ev = new WeatherChangeEvent($this, self::WEATHER_IMAGICALMINE));
					if(!$ev->isCancelled()){
			...
			if($this->getWeather() === self::WEATHER_RAIN){
				if($this->weatherexectick <= 0){
					...
					if(!$ev->isCancelled()){
						...
						$this->setWeather(self::WEATHER_IMAGICALMINE);
						return;
					}
				}
	...
	public function broadcastWeather($weather = self::WEATHER_IMAGICALMINE,Player $player=null){
		$pk = new LevelEventPacket();
		switch($weather){
			case self::WEATHER_IMAGICALMINE :
	...
	public function setWeather($weather = self::WEATHER_IMAGICALMINE,$time = null){
		if($time !== null){
